...
class zzLiberoDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
    ...
    def _merge_replay_buffers(self, zarr_paths: List[str]) -> ReplayBuffer:
        ...
        for path in zarr_paths:
            ...
            buffer = ReplayBuffer.copy_from_path(path, keys=[ 'actions', 'pointcloud', 'joint_states', 'gripper_states','robot_states'])
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        agent_pos = np.concatenate([self.replay_buffer['joint_states'][..., :7], self.replay_buffer['gripper_states'][..., :2]], axis=-1)
        
        data = {
            'action': self.replay_buffer['actions'],
            'agent_pos': agent_pos,  # 这里是拼接的gripper和joint状态
