...
class zzCalvinDataset(BaseDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
    ...
    def _sample_to_data(self, sample):
        # import pdb; pdb.set_trace()
        agent_pos = sample['states'][:, -18:-9].astype(np.float32)  # 取倒数第18到倒数第10，共9个
        point_cloud = sample['pointclouds'][:,].astype(np.float32) # (T, 1024, 6)
